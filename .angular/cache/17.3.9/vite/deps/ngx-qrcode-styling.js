...
var ChangeDetectionSchedulerImpl = class _ChangeDetectionSchedulerImpl {
  constructor() {
    ...
    this.scheduleInRootZone = !this.zonelessEnabled && this.zoneIsDefined && (inject(SCHEDULE_IN_ROOT_ZONE, { optional: true }) ?? false);
...
var GlobalEventDelegation = class _GlobalEventDelegation {
  ...
  static {
    this.ɵprov = ɵɵdefineInjectable({ token: _GlobalEventDelegation, factory: _GlobalEventDelegation.ɵfac });
  }
};
(() => {
...
function drawQrcode(config2, container) {
  ...
  const QRCODE_NONE_FRAME = () => {
    if (config2?.frameOptions) {
      return false;
    } else {
      const encodeConfig = () => {
        let deep = config2 && JSON.parse(JSON.stringify(config2));
  ...
  const styleName = config2?.frameOptions?.style ?? "F_020";
  const height = config2?.frameOptions?.height ?? 300;
  const width = config2?.frameOptions?.width ?? 300;
  const x = config2?.frameOptions?.x ?? 50;
  const y = config2?.frameOptions?.y ?? 50;
  ...
  const upgradeSvg = (result) => {
    ...
    if (doc) {
      ...
      config2?.frameOptions?.background && svgEl.setAttribute("style", `background: ${config2?.frameOptions?.background};${svgEl?.getAttribute("style")}`);
      const updateStyle = (el, config3) => {
        if (el) {
          for (const key in config3) {
            if (["x", "y", "transform"].includes(key)) {
              el.setAttribute(key, config3 && config3[key]);
            } else if (["textContent"].includes(key)) {
              el[key] = config3 && config3[key];
            } else {
              el.style[key] = config3 && config3[key];
            }
          }
        }
      };
      const createElementNS = (config3) => {
        const svgNS = "http://www.w3.org/2000/svg";
        const newText = document.createElementNS(svgNS, "text");
        updateStyle(newText, config3);
        svgEl.appendChild(newText);
      };
      if (config2?.frameOptions?.texts) {
        [...config2.frameOptions.texts].forEach((text, i) => {
          const el = [...textEls]?.[i];
          el ? updateStyle(el, text) : createElementNS(text);
        });
      }
      if (config2?.frameOptions?.containers) {
        [...containerEls].forEach((el, i) => {
          updateStyle(el, config2?.frameOptions?.containers?.[i]);
        });
      }
      if (config2?.frameOptions?.contents) {
        [...contentEls].forEach((el, i) => {
          updateStyle(el, config2?.frameOptions?.contents?.[i]);
        });
      }
  ...
  const UPDATE_ROTATE_SCALE_QRCODE_ON_FRAME = (svg) => {
    if (svg && config2?.rotate) {
      svg?.childNodes?.[0]?.childNodes?.forEach((node) => {
        if (node.nodeName === "rect") {
          node.style.transformOrigin = `50% 50%`;
          node.style.transform = `rotate(${config2?.rotate ?? 0}deg)`;
        }
      });
    }
    if (svg && config2?.scale) {
      svg?.childNodes?.[0]?.childNodes?.forEach((node) => {
        if (node.nodeName === "rect") {
          node.style.scale = config2?.scale ?? 0;
        }
      });
    }
  };
...
var defaultTemplate = (config2) => {
  let deep = config2 && JSON.parse(JSON.stringify(config2));
  return config2?.template ? __spreadValues(__spreadValues({}, Templates(config2.template.toLocaleLowerCase())), deep) : deep;
};
var deepUpdate = (config2, configUpdate) => __async(void 0, null, function* () {
  const origin = config2 && JSON.parse(JSON.stringify(config2));
...
var NgxQrcodeStylingService = class _NgxQrcodeStylingService {
  /**
   * create
   * @param config
   * @param container
   * @returns
   */
  create(config2, container) {
    return drawQrcode(defaultTemplate(config2), container);
  }
  /**
   * update
   * @param config
   ...
   * @returns
   */
  update(config2, configUpdate, container) {
    const subject = new AsyncSubject();
    (function() {
      return __async(this, null, function* () {
        const conf = yield deepUpdate(defaultTemplate(config2), defaultTemplate(configUpdate));
   ...
   * @returns
   */
  download(container, fileName = "", timeout2 = 0) {
    const subject = new AsyncSubject();
  ...
  static {
    this.ɵfac = function NgxQrcodeStylingService_Factory(t) {
...
var NgxQrcodeStylingComponent = class _NgxQrcodeStylingComponent {
   ...
   */
  create(config2) {
    return this.service.create(config2, this.canvas.nativeElement);
  }
  /**
   * update
   * @param config
   ...
   * @returns
   */
  update(config2, configUpdate) {
    return this.service.update(config2, configUpdate, this.canvas.nativeElement);
  }
  /**
   * download image
   * @param fileName eg: demo.png
   ...
   * @returns
   */
  download(fileName, timeout2 = 0) {
    return this.service.download(this.canvas.nativeElement, fileName, timeout2);
  }
  static {
    this.ɵfac = function NgxQrcodeStylingComponent_Factory(t) {
...
var NgxQrcodeStylingModule = class _NgxQrcodeStylingModule {
  static {
    this.ɵfac = function NgxQrcodeStylingModule_Factory(t) {
